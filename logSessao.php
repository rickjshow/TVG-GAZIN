...
<body>
    ...
    <div class="container mt-4 mb-3">
        <div class="box1 mt-4 text-center p-4 border rounded shadow">
            <h3 class="mt-4 font-weight-bold display-4 text-primary" style="font-size: 18px;">Log de Sessões</h3>
                <form method='post' action='logSessao.php' class='mt-4'>
                    <div class='form-row align-items-center justify-content-center'>
                        <div class='col-auto'>
                        </div>
                        <div class='col-auto'>
                            <label class="mt-2" for='tvg'>Selecione a sessão:</label><br>
                            <select class='custom-select mr-sm-2' name='sessao' id='user' style='width: 200px;'>                                  
                                <?php
                                   ...
                                   echo "<option value='Todas as sessoes'";
                                   if ($nomeSelecionado == 'Todas as sessoes') {
                                       echo " selected";
                                   }
                                   echo ">Todas as sessoes</option>";
                        ...
                        <div class='col-auto'>
                            ...
                            <select class='custom-select mr-sm-2' name='usuario' id='user' style='width: 200px;'>                                  
                                <?php
                                   ...
                                   $queryUser = $pdo->prepare($query);
                                   $queryUser->execute();
                                   $SessaoName = $queryUser->fetchAll(PDO::FETCH_ASSOC);
            ...
            <tbody>
                <?php
                    if(isset($_POST['filtrar'])){
                        ...
                        $querySessao = "SELECT id FROM sessoes WHERE nome = ?";
                        $result = $pdo->prepare($querySessao);
                        $result->bindValue(1, $sessao);
                        $result->execute();
                        $idSessao = $result->fetchColumn();

                        $queryLog = "SELECT log_sessoes.*, usuarios.nome AS nome_usuario FROM log_sessoes
                        LEFT JOIN usuarios ON log_sessoes.id_usuarios = usuarios.id 
                        LEFT JOIN sessoes ON log_sessoes.id_sessoes = sessoes.id
                        WHERE 1=1";
           
                        $bindings = array();

                        if (!empty($dataInicial) && !empty($dataFinal)) {
                            $queryLog .= " AND log_sessoes.horario BETWEEN ? AND ?";
                            $bindings[] = $dataInicial;
                            $bindings[] = $dataFinal;
                        } elseif (!empty($dataInicial) && empty($dataFinal)) {
                            $queryLog .= " AND log_sessoes.horario >= ?";
                            $bindings[] = $dataInicial;
                        ...
                        if ($usuario != 'Todos os usuarios') {
                            $queryLog .= " AND log_sessoes.id_usuarios = ?";
                            $bindings[] = $idUser;
                        }

                        if($sessao != 'Todas as sessoes'){
                            $queryLog .= " AND log_sessoes.id_sessoes = ?";
                            $bindings[] = $idSessao;
                        } 
